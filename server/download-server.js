...
const RELEASE_CONFIG = {
  changelog: "v1.128 彻底修复电流声（帧格式标志升级，兼容版本错配）\n① 根因：v1.127 用帧长判断 ADPCM 帧格式，双端版本不一致（一端旧版发 481 字节帧）会被误判解码偏移，持续输出电流声\n② 升级：帧首字节改用独立标志（0x02=新格式），接收端按标志识别，任何版本组合都不再错位\n③ 新增：诊断上报含版本号，便于确认两端实际安装版本\n④ 重要：本版 ADPCM 帧格式升级，两端需同步更新到 v1.128",
  forced: false,
};
