...
const RELEASE_CONFIG = {
  changelog: "v1.131 电流声诊断：帧级字节对比定位失真环节\n① v1.130 波形快照发现：采集端波形正常，但播放端解码输出指数发散到满幅（rms 从正常值 4700 突变到 32000+），且播放帧数比发送帧数多约 10 帧\n② 本版上报编码端/解码端的帧头预测器状态（encState/decState）与帧前 8 字节（encHead/decHead），逐字节对比确认传输是否破坏 ADPCM 数据，同时确认编码器状态是否在帧间正确延续\n③ 若双端 head 一致则传输无损、问题在解码器状态；若 head 不一致则传输环节破坏帧字节\n④ 请双端更新后测试，测试后我读取诊断日志直接对比字节",
  forced: false,
};
