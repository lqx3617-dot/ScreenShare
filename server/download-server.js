...
const RELEASE_CONFIG = {
  changelog: "v1.130 电流声诊断增强：上报波形快照定位问题\n① 上一版诊断数据发现：采集端信号正常但播放端持续满幅（rms≈27000），解码输出远超采集信号，疑似传输/解码环节失真\n② 本版在采集端与播放端各上报最近一帧前 16 个 PCM 采样值快照，精确对比两端波形，定位失真发生在哪一环\n③ 同时上报接收帧格式（长度/首字节），确认双端 ADPCM 版本一致\n④ 请双端更新后测试，测试后我读取诊断日志直接分析波形",
  forced: false,
};
